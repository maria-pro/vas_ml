...
data_stephanie<-data_stephanie%>%mutate(
  LoS_cat_120=as_factor(
    case_when(lo_s>120~"1",
                        TRUE~"0")
)
)
...
data_model_stay120<-data_stephanie%>%
  select(
    "client_id",
     "sex",
  "marital_status",
...
describe(data_model_stay120)%>%gt()

skimr::skim(data_model_stay120)
DataExplorer::plot_bar(data_model_stay120)
...
data_split<-initial_split(data_model_stay120, strata=LoS_cat_120) #prop = 3/4
data_train<-training(data_split)
data_test<-testing(data_split)

data_folds <- vfold_cv(data_train, v = 5, strata = LoS_cat_120, repeats = 5)
data_boot <- bootstraps(data_model_stay120)

data_model_stay120%>%count(LoS_cat_120)
levels(data_train$LoS_cat_120)

data_stay120_rec <- recipe(LoS_cat_120 ~ ., data = data_train) %>%
...
### Vas code
```{r eval=FALSE}
mice::md.pattern(data_model_stay120, rotate.names = TRUE)
```

```{r}

naniar::mcar_test(data_model_stay120)
...
```{r}
results_NF <- data_test%>%select(LoS_cat_120)%>% 
  bind_cols(Null_fit%>% 
...
results_NF %>% 
  conf_mat(truth = LoS_cat_120, estimate = .pred_class)
```

```{r}
update_geom_defaults(geom = "rect", new = list(fill = "midnightblue", alpha = 0.7))
results_NF%>% 
  conf_mat(LoS_cat_120,.pred_class) %>% 
  autoplot()
```

```{r}
ev_met1_NF<-ev_met1(results_NF,truth = LoS_cat_120, estimate = .pred_class)
Acc_NF<-yardstick::accuracy(results_NF, LoS_cat_120,.pred_class)
Rec_NF<-yardstick::recall(results_NF, LoS_cat_120, .pred_class)
#Plot Roc_Curve
curve_NF <- results_NF %>% 
  roc_curve(LoS_cat_120, .pred_0) %>% 
  autoplot
```
...
```{r}
library(knitr)
results_RF <- data_test %>% select(LoS_cat_120)%>% 
  bind_cols(RF_fit %>% 
              predict(new_data = data_test))%>% 
  bind_cols(RF_fit%>% 
             predict(new_data = data_test, type = "prob"))
...
```{r}
ev_met1_RF<-ev_met1(results_RF,truth = LoS_cat_120, estimate = .pred_class)
Rec_RF<-yardstick::recall(results_RF, LoS_cat_120, .pred_class)
ACC_RF<-yardstick::accuracy(results_RF, LoS_cat_120, .pred_class)
#Plot Roc_Curve
curve_RF <- results_RF %>% 
  roc_curve(LoS_cat_120, .pred_0) %>% 
  autoplot
...
```{r}
results_RFW <- data_model_stay120 %>% select(LoS_cat_120)%>% 
  bind_cols(RF_fit %>% 
              predict(new_data = data_model_stay120))%>% 
  bind_cols(RF_fit%>% 
             predict(new_data = data_model_stay120, type = "prob"))
kable(results_RFW)
```
...
```{r}
ev_met1_RFW<-ev_met1(results_RFW,truth = LoS_cat_120, estimate = .pred_class)
Rec_RFW<-yardstick::recall(results_RFW, LoS_cat_120, .pred_class)
ACC_RFW<-yardstick::accuracy(results_RFW, LoS_cat_120, .pred_class)
...
results_LR <- data_test %>% select(LoS_cat_120) %>% 
...
```{r}
ev_met1_LR<-ev_met1(results_LR,truth = LoS_cat_120, estimate = .pred_class)
Rec_LR<-yardstick::recall(results_LR, LoS_cat_120, .pred_class)
ACC_LR<-yardstick::accuracy(results_LR, LoS_cat_120, .pred_class)
#Plot Roc_Curve
curve_LR <- results_LR %>% 
  roc_curve(LoS_cat_120, .pred_0) %>% 
  autoplot
...
```{r}
results_LRW <- data_model_stay120 %>% select(LoS_cat_120)%>% 
  bind_cols(LogReg_fit %>% 
              predict(new_data = data_model_stay120))%>% 
  bind_cols(LogReg_fit%>% 
             predict(new_data = data_model_stay120, type = "prob"))
...
```{r}
ev_met1_LRW<-ev_met1(results_LRW,truth = LoS_cat_120, estimate = .pred_class)
Rec_LRW<-yardstick::recall(results_LRW, LoS_cat_120, .pred_class)
ACC_LRW<-yardstick::accuracy(results_LRW, LoS_cat_120, .pred_class)
#Plot Roc_Curve
...
```{r}
results_Lasso<-data_test %>% select(LoS_cat_120) %>% 
  bind_cols( Lasso_fit%>% 
              predict(new_data = data_test)) %>% 
  bind_cols( Lasso_fit%>% 
             predict(new_data = data_test, type = "prob"))
...
```{r}
ev_met1_Lasso<-ev_met1(results_Lasso,truth = LoS_cat_120, estimate = .pred_class)
ACC_Lasso<-yardstick::accuracy(results_Lasso, LoS_cat_120,.pred_class)
Rec_Lasso<-yardstick::recall(results_Lasso, LoS_cat_120, .pred_class)
#Plot Roc_Curve
...
```
```{r}
results_NB <- data_test %>% select(LoS_cat_120) %>% 
 bind_cols(NB_fit %>% 
             predict(new_data = data_test)) %>% 
  bind_cols( NB_fit%>% 
              predict(new_data = data_test, type = "prob"))
...
ev_met1_NB<-ev_met1(results_NB,truth = LoS_cat_120, estimate = .pred_class)
Rec_NB<-yardstick::recall(results_NB, LoS_cat_120, .pred_class)
ACC_NB<-yardstick::accuracy(results_NB, LoS_cat_120, .pred_class)
#Plot Roc_Curve
curve_NB <- results_NB %>% 
  roc_curve(LoS_cat_120, .pred_0) %>% 
  autoplot
...
```{r}
results_Kern<-data_test %>% select(LoS_cat_120) %>% 
...
ev_met1_Kern<-ev_met1(results_Kern,truth = LoS_cat_120, estimate = .pred_class)
Rec_Kern<-yardstick::recall(results_Kern, LoS_cat_120, .pred_class)
ACC_Kern<-yardstick::accuracy(results_Kern, LoS_cat_120, .pred_class)
#Plot Roc_Curve
...
results_tuned_xgb<-data_test %>% select(LoS_cat_120) %>% 
...
ev_met1_Tunxgb<-ev_met1(results_tuned_xgb,truth = LoS_cat_120, estimate = .pred_class)
Rec_Txgb<-yardstick::recall(results_tuned_xgb, LoS_cat_120, .pred_class)
ACC_Txgb<-yardstick::accuracy(results_tuned_xgb, LoS_cat_120, .pred_class)
...
results_tuned_LR<-data_test %>% select(LoS_cat_120) %>% 
...
```{r}
ev_met1_TunLR<-ev_met1(results_tuned_LR,truth = LoS_cat_120, estimate = .pred_class)
Rec_TLR<-yardstick::recall(results_tuned_LR, LoS_cat_120, .pred_class)
ACC_TLR<-yardstick::accuracy(results_tuned_LR, LoS_cat_120, .pred_class)
...
results_tuned_NB<-data_test %>% select(LoS_cat_120) %>% 
...
ev_met1_TunNB<-ev_met1(results_tuned_NB,truth = LoS_cat_120, estimate = .pred_class)
Rec_TNB<-yardstick::recall(results_tuned_NB, LoS_cat_120, .pred_class)
ACC_TNB<-yardstick::accuracy(results_tuned_NB, LoS_cat_120, .pred_class)
...
#Test the tuned AI on the test data
results_tuned_RF<-data_test %>% select(LoS_cat_120) %>% 
...
auc_RF_tuned <- results_tuned_RF %>% 
  roc_auc(LoS_cat_120, .pred_0)
ev_met1_TunRF<-ev_met1(results_tuned_RF,truth = LoS_cat_120, estimate = .pred_class)
Rec_TRF<-yardstick::recall(results_tuned_RF, LoS_cat_120, .pred_class)
ACC_TRF<-yardstick::accuracy(results_tuned_RF, LoS_cat_120, .pred_class)
...
#Test the tuned AI on the test data
results_tuned_Lasso<-data_test%>% select(LoS_cat_120) %>% 
...
ev_met1_TunLasso<-ev_met1(results_tuned_Lasso,truth = LoS_cat_120, estimate = .pred_class)
Rec_TLasso<-yardstick::recall(results_tuned_Lasso, LoS_cat_120, .pred_class)
ACC_TLasso<-yardstick::accuracy(results_tuned_Lasso, LoS_cat_120, .pred_class)
...
k_grid <- tibble(neighbors = c(10, 15, 25, 45, 60, 80, 100, 120, 140, 120))
...
results_tuned_knn<-data_test%>% select(LoS_cat_120) %>% 
...
ev_met1_Tunknn<-ev_met1(results_tuned_knn,truth = LoS_cat_120, estimate = .pred_class)
Rec_Tknn<-yardstick::recall(results_tuned_knn, LoS_cat_120, .pred_class)
ACC_Tknn<-yardstick::accuracy(results_tuned_knn, LoS_cat_120, .pred_class)
...
#Test the tuned AI on the test data
results_tuned_SVM<-data_test %>% select(LoS_cat_120) %>% 
...
ev_met1_TunSVM<-ev_met1(results_tuned_SVM,truth = LoS_cat_120, estimate = .pred_class)
Rec_TSVM<-yardstick::recall(results_tuned_SVM, LoS_cat_120, .pred_class)
ACC_TSVM<-yardstick::accuracy(results_tuned_SVM, LoS_cat_120, .pred_class)
